...
api_url = 'http://localhost:8000/app12_1/customers/'

def index(request):
    response = requests.get(api_url)
    records = response.text
    records = json.loads(records) #Text to Dictionary
    return render(request, 'app12_2/index.html', {'persons': records})
...
def display_new_form(request):
    if request.method =='POST':
        ...
        if response.status_code == 201:
            ...
            result = False
        return render(request, 'app12_2/result.html', {'title': 'Record insert!', 'status': result})
    return render(request, 'app12_2/new.html')
...
def update(request):
    if request.method == 'POST':
        ...
        if response.status_code == 200:
            ...
            result = False
        return render(request, 'app12_2/result.html', {'title': 'Record update!', 'status': result})
    ...
    print(type(record))
    print(record)
    return render(request, 'app12_2/edit.html', {'contact': record})
...
def delete(request):
    ...
    if response.status_code == 204:
        ...
        result = False
    return render(request, 'app12_2/result.html', {'title': 'Record delete!', 'status': result})